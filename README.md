...
- **解答题和证明题是「自评」，不是「AI 判卷」。** 248 道内置题里，客观题 228 道
  （单选 173 / 填空 31 / 多选 12 / 判断 12），主观题 20 道（解答 12 / 证明 8）。
  主观题走「亮参考答案 → 用户对照评分点自评」两步，服务端只认用户的自评结果。
  > 这条线的位置是**有意**划的：解答题的过程没法用字符串比对判，硬塞进自动判分
  > 只会得到「我明明做对了它说错」。自评至少是诚实的 —— 而且评分点是逐条列的，
  > 比「看一眼参考答案然后说差不多对了」严谨。
  >
  > 真要自动判，可行的方向是「手写/拍照 → AI 判卷 → 人工确认」，
