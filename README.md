...
题库补到了 **248 道**，题型从 2 种扩到 **6 种**（单选 / 多选 / 填空 / 判断 / 解答 / 证明）—— 补齐了考研卷面上占 70 分的解答题与证明题，见[六种题型](#六种题型解答题为什么不能自动判分)。
...
│   │   └── data/               3 科 19 章 68 考点 248 题 + 257 条公式 + edges.json（102 条图谱边）
│   └── scripts/
...
新增 44 道：多选 12 · 判断 12 · 解答 12 · 证明 8，覆盖高数 / 线代 / 概率三科。
**每道解答题都带评分点**，否则「分步给分」无从谈起。
...
内置题库只有 248 道，而错题重做三遍，记住的是「这道题的答案」，不是「这类题的方法」—— 换个数字照样错。变式题给的是后者。
...
内置题库只有 248 道，AI 变式题也只是「同一考点的变体」。真正的短板在**真题卷子**上 —— 那些题原来一道都进不来。
...
- **模型输出**：数学写在 `$...$` / `$$...$$` 里
- **库里的数据**：**裸 LaTeX，一个 `$` 都没有** —— 68 个知识点的正文与例题、248 道题的题干与解析，全是「中文句子里夹着 `\frac{...}`」
...
- **渲染套件**（`latex-coverage.mjs` + `pipeline-leak.mjs`）：用真 katex 跑完整 `renderRich`，全量扫 248 题 + 68 知识点共 1400+ 段文本，检查两条不变量 —— KaTeX 解析失败 = 0、裸命令漏屏 = 0。管线代码是**从 `Math.tsx` 现场抽**的（调 tsc），不手抄副本。
...
- **解答题和证明题靠 AI 批改 + 用户确认，不是全自动判卷。** 248 道内置题里，
  客观题 228 道（单选 173 / 填空 31 / 多选 12 / 判断 12），主观题 20 道（解答 12 / 证明 8）。
