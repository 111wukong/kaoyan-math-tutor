...
│       └── pages/              17 个页面
└── tests/
    ├── browser-smoke.mjs       真浏览器冒烟（225 项，CDP，零依赖）
    ├── screenshot.mjs          逐页截图（npm run shots），支持 THEME= 换主题 / after 钩子滚动
...
内置题库只有 204 道，AI 变式题也只是「同一考点的变体」。真正的短板在**真题卷子**上 —— 那些题原来一道都进不来。
...
- **浏览器套件**（`tests/browser-smoke.mjs`）：用系统里已有的 Chromium 内核 + Node 22 自带的 `WebSocket` 说 CDP，**零 npm 依赖**。真开页面走完 14 个路由，断言 canvas 真的画了东西（数非透明像素）、切模块后画布真的重画、拖滑块读数真的变、公式真的渲染且没漏源码、**切主题后底色与文字对比度真的翻过来**、**管理台普通用户进不去而管理员进得去**、**切页不卸载也不重拉接口（数 CDP 收到的请求条数）**、**点目录能直达考点**、**说「不知道」之后三个同学真的闭嘴了（对着假模型数气泡）**。找不到浏览器会优雅跳过。
